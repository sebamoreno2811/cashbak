...
export default function StoreManager({
  ...
  async function saveDeliveryOptions() {
    setSavingDelivery(true); setDeliveryError(null)
    const res = await updateStoreDeliveryOptions(deliveryOptions)
    if (res.error) {
      setDeliveryError(res.error)
    } else {
      setSavedDeliveryOptions(deliveryOptions); setDeliverySaved(true)
  ...
  function openEdit(p: StoreProduct) { setEditing(p); setShowForm(true) }
  ...
  return (
    <div className="flex bg-gray-50 min-h-[calc(100vh-64px)]">
      ...
      {/* ── Desktop sidebar ───────────────────────────────────────────────── */}
      <aside className="hidden md:flex flex-col w-56 bg-green-900 shrink-0 sticky top-0 h-[calc(100vh-64px)] overflow-y-auto border-r border-green-800">

        {/* Store identity */}
        <div className="px-4 py-5 border-b border-green-800">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-white overflow-hidden flex items-center justify-center shrink-0 ring-2 ring-green-700">
              {store.logo_url ? (
                <Image src={store.logo_url} alt={store.name} width={40} height={40} className="object-cover w-full h-full" />
              ) : (
                <Image src="/img/logo.png" alt="CashBak" width={40} height={40} className="object-contain w-7 h-7" />
              )}
            </div>
        ...
        <nav className="flex-1 p-3 space-y-0.5 pt-4">
          ...
          <div className="pt-4">
            <p className="text-green-500 text-[10px] font-semibold uppercase tracking-wider px-3 mb-2">Tu tienda</p>
            <SidebarItem
              icon={Building2}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Mobile top bar */}
        <div className="md:hidden bg-green-900 text-white px-4 pt-4 pb-0">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-8 h-8 rounded-full bg-white overflow-hidden flex items-center justify-center shrink-0">
              {store.logo_url ? (
                <Image src={store.logo_url} alt={store.name} width={32} height={32} className="object-cover w-full h-full" />
              ) : (
                <Image src="/img/logo.png" alt="CashBak" width={32} height={32} className="object-contain w-6 h-6" />
              )}
            </div>
            ...
            <>
              {products.length === 0 && !showForm && (
                <div className="text-center py-20 text-gray-400">
                  ...
                  <button
                    onClick={openAdd}
                    className="mt-6 inline-flex items-center gap-2 bg-green-900 text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-green-800 transition-colors cursor-pointer"
                  >
          ...
          {activeTab === "pago" && (
            <div className="max-w-lg">
              <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-5">
                ...
                <button
                  onClick={async () => {
                    setSavingBank(true); setBankError(null)
                    const res = await updateStoreBankAccount(bank)
                    if (res.error) setBankError(res.error)
                    else { setBankSaved(true); setTimeout(() => setBankSaved(false), 2500) }
                    setSavingBank(false)
                  }}
                ...
                >
                  {savingBank ? <><Loader2 className="w-4 h-4 animate-spin" /> Guardando...</>
                    : bankSaved ? <><CheckCircle2 className="w-4 h-4" /> ¡Guardado!</>
              ...
              <div className={`bg-white rounded-xl border p-5 space-y-4 ${deliveryHasChanges ? "border-amber-300 shadow-amber-100 shadow-md" : "border-gray-200"}`}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  {deliverySaved && !deliveryHasChanges && (
                    <span className="shrink-0 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-1.5">
                      ✓ Cambios guardados
                    </span>
                  )}
                </div>
                ...
                {deliveryHasChanges && !showAddOpt && (
                  <div className="border-t border-amber-200 pt-4 space-y-3">
                    ...
                    <button onClick={saveDeliveryOptions} disabled={savingDelivery}
                      className="w-full py-3 bg-green-900 text-white rounded-xl font-bold hover:bg-green-800 transition-colors disabled:opacity-50 text-sm cursor-pointer">
...
function ProductFormModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    for (const slot of imageSlots) {
      ...
      if (upErr) { setError(`Error al subir imagen: ${upErr.message}`); setUploading(false); setSaving(false); return }
    ...
    const res = initial ? await updateProduct(initial.id, payload) : await addProduct(payload)
    if (res.error) { setError(res.error); setSaving(false); return }

    onSaved({
      id: initial?.id ?? 0, name: payload.name, price: payload.price, cost: payload.cost,
      margin_pct: payload.margin_pct, category_name: payload.category_name,
      category_names: payload.category_names, description: payload.description || null,
      image: imageUrl, images: finalImages, stock: stockPayload,
    })
    setSaving(false)
  }
        ...
        <form onSubmit={handleSubmit} className="px-5 py-5 space-y-5">
            ...
            <div className="grid grid-cols-4 gap-2">
              ...
              {imageSlots.length < 4 && (
                <button type="button" onClick={() => fileRef.current?.click()}
                  className="aspect-square rounded-xl border-2 border-dashed border-gray-200 hover:border-green-600 flex flex-col items-center justify-center gap-1 text-gray-400 hover:text-green-700 transition-colors cursor-pointer">
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {categoryNames.map(cat => (
                  <span key={cat} className="flex items-center gap-1 bg-green-900 text-white text-xs font-medium px-2.5 py-1 rounded-full">
                    {cat}
                    <button type="button" onClick={() => setCategoryNames(categoryNames.filter(c => c !== cat))} className="hover:text-green-300 leading-none cursor-pointer">×</button>
                  </span>
                ))}
              </div>
            )}
            {categoryNames.length < 3 && (
              <select value="" onChange={e => { if (e.target.value && !categoryNames.includes(e.target.value)) setCategoryNames([...categoryNames, e.target.value]) }}
              ...
              {bets.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {bets.map(bet => (
                      <button key={bet.id} type="button" onClick={() => setSelectedBetId(bet.id)}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex rounded-lg overflow-hidden border border-gray-200 text-xs">
                <button type="button" onClick={() => setStockMode("sizes")}
                  className={`px-3 py-1.5 font-medium transition-colors cursor-pointer ${stockMode === "sizes" ? "bg-green-900 text-white" : "bg-white text-gray-500 hover:bg-gray-50"}`}>
                  Con tallas
                </button>
                <button type="button" onClick={() => setStockMode("single")}
                  className={`px-3 py-1.5 font-medium transition-colors cursor-pointer ${stockMode === "single" ? "bg-green-900 text-white" : "bg-white text-gray-500 hover:bg-gray-50"}`}>
                  Sin tallas
                </button>
              </div>
            </div>
